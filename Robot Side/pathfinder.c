...
void flood_fill(dmap * map, int row, int column, coordinate_node * reachable) {
    char symbol = get_symbol_dmap(map, row, column);
    ...
    flood_fill(map, row+1, column, reachable);     // North
    flood_fill(map, row-1, column, reachable);     // South
    flood_fill(map, row, column-1, reachable);     // West
    flood_fill(map, row, column+1, reachable);     // East
}

coordinate_node * get_unexplored_coordinates(dmap * map, int row, int column) {
    coordinate_node * reachable;
    reachable = (coordinate_node *) malloc(sizeof(coordinate_node));
    new_coordinate_list(reachable);

    flood_fill(map, row, column, reachable);
...
coordinate_node * get_nearest_unexplored(dmap * map, coordinate_node * unexploredCoordinates, int row, int column) {
    // Initial coordinate
    Coordinate c = {row, column};
...
void parse_path(coordinate_node * path, Robot * robot) {
	sound_node * seq = (sound_node *) malloc(sizeof(sound_node));
	new_sound_list(seq);
//    char * seq = (char*) malloc(sizeof(char));
    ...
    while (current->next != NULL) {
        ...
        if (r == 1) {
        	push_sound_list(&seq, 1);
        }
//        	seq[len] = 'W';
        else if (c == -1){
        	push_sound_list(&seq, 2);
        }
//        	seq[len] = 'A';
        else if (r == -1){
        	push_sound_list(&seq, 3);
        }
//        	seq[len] = 'S';
        else if (c == 1){
        	push_sound_list(&seq, 4);
        }
//        	seq[len] = 'D';
    ...
    while(seq -> next != NULL) {
    	...
    	chThdSleepMilliseconds(100);
//    for(int i = 0; i < len; i++)
    		int movement = pop_last_sound_list(seq);
    		switch(movement){
				case 1:
...
coordinate_node * find_path(dmap * map, Coordinate s, Coordinate t) {
    coordinate_node * unexplored;
    ...
    return get_nearest_unexplored(map, unexplored, row, column);
}

void move_robot_in_map(dmap * map, int id, Coordinate s, Coordinate t) {

	if (compare_coordinate(&s, &t))
	        return;

    coordinate_node * path = find_path(map, s, t);
    coordinate_node * current = path;
...
void robot_moved_in_map(dmap * map, int id, Coordinate target) {
    char idc = (char) (id - 208);

    for (int i = 0; i < HEIGHT; i++)
        for (int j = 0; j < WIDTH; j++) {
            Coordinate c = {i, j};
            if (get_symbol_dmap(map, i, j) == idc && !compare_coordinate(&c, &target))
            	set_symbol_dmap(map, i, j, 'o');
        }
}

Coordinate * get_nearest_coordinate(dmap * map, int row, int column) {
	coordinate_node * unexplored;
    unexplored = get_unexplored_coordinates(map, row, column);
    ...
    return coordinate_list_last(get_nearest_unexplored(map, unexplored, row, column));
