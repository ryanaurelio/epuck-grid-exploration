...
void move_robot_in_dmap(dmap map, int id, Coordinate t) {
	Coordinate s = {get_robot_with_index(id - 1) -> coordinate->x, get_robot_with_index(id - 1) -> coordinate->y};
	get_robot_with_index(id - 1) -> coordinate->x = t.x;
	get_robot_with_index(id - 1) -> coordinate->y = t.y;
...
void robot_moved_in_dmap(dmap map, int id) {
	Coordinate target = {get_robot_with_index(id - 1) -> coordinate -> x, get_robot_with_index(id - 1) -> coordinate -> y};
	push_to_free_robots_list(get_robot_with_index(id - 1));
    char idc = (char) (id - 208);
...
Coordinate * get_nearest_coordinate_dmap(dmap map) {
	int x = get_robot_with_index(get_my_id() - 1)->coordinate -> x;
	int y = get_robot_with_index(get_my_id() - 1)->coordinate -> y;
    coordinate_node * reachable;
    reachable = (coordinate_node *) malloc(sizeof(coordinate_node));
    new_coordinate_list(reachable);
